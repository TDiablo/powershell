...
if (!$group)
{ $account = “\\$ComputerName\root\cimv2:Win32_Account.Name=’$Name’,Domain=’$Domain’” }
else
{ $account = “\\$ComputerName\root\cimv2:Win32_Group.Name=’$Name’,Domain=’$Domain’” }

#Get the SID for the found account.
$accountSID = “\\$ComputerName\root\cimv2:Win32_SID.SID=’$($account.sid)’”
...
$ACE = @(New-ACE -Name “wfeuser1” -Domain “GLOBAL.PMIHQ.ORG” -Permission “Full”)
$ACE += New-ACE -Name “everyone” -Domain “.” -Permission “Read” -Group

#Create the share on the local machine
$result = New-Share -FolderPath “D:\myPMI” -ShareName “myPMI" -ACEs $ACE -Description “” -Computer “files2.production.pmi.org”
